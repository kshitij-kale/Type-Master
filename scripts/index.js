...
let result = document.querySelector('.grid-item-5');
let scoreBoard = document.querySelector('.grid-item-6');
let started = true;
...
for(let i = 0; i < 500; i++) {
  ...
  let charArray = wordArray[rand].split('');
  charArray.forEach((char) => {
    wordBox.innerHTML += `<span>${char}</span>`;
  });
...
function match(e) {
  ...
  let lastChar = typingBox.value.slice(-1);
  let currSpan = wordBox.children[counter];
  if(currSpan.textContent == lastChar) {
    if(currSpan.textContent == ` `) {
      numCompletedWords++;
    }
    currSpan.style.color = 'green';
    counter++;
  }
  else {
    currSpan.style.color = 'red';
  }
}
...
if(time != 0) {
    ...
    result.textContent = `${numCompletedWords * 2} Words Per Minute`;
    if(numCompletedWords * 2 > localStorage.getItem("hs")) {
      localStorage.setItem("hs", numCompletedWords * 2 );
      scoreBoard.textContent = `High Score: ${localStorage.getItem("hs")} Words Per Minute`;
    }
    typingBox.remove();
